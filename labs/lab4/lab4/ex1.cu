...
// Contains both exercies 1 and 2
...
__global__ void calcHistogramKernel(uchar4* gray_image, hist_t* histogram_device, int width, int height) {
    ...
    if (x < width && y < height) {
        unsigned char gray = gray_image[index].x;
        atomicAdd(&(histogram_device[gray]), 1);
    }
}
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        if (output2 != nullptr) {
            ...
            // plot histogram
            plotHistogramKernel<<<(256+255)/256, 256>>>(out_image, histogram_device, input->GetWidth(), input->GetHeight(), max_freq);
